...
//Analysiert die LogDatei und �ndert dem entsprechend Players
void FileTransformer(const char *LogFile, Spieler *Players)
{
	...
	if(LogDatei.open(IO_ReadOnly))
	{
		...
		//Anonyme Map setzen, wird sp�ter ggf. ge�ndert
			...
			if(strlen(Line) > 26)
			{
				...
				if(Line[25] == '\"')
				{
					...
					if(strcmp(action, " changed name to ") == 0)
					{
						...
						//Auch in AktNamen und Gekillt �ndern
					...
					{
						//Wenn jemand gekillt wurde, hier werden Name, Waffe gefiltert und dann an Spieler::NewKills �bergeben
						...
						//Ab hier wird der Name des get�teten Spielers in WasKilledName geschrieben
						strcpy(Buffer, Line);
						...
						for(z = strlen(Buffer) - 1; z >= 0; z--)
						{
							...
							{
        						temp = &Buffer[z+1];
								//Wird sp�ter f�r den Namens des Killers gebraucht
								nachkilled = z - 1;
								break;
							}     	
						}
						...
						{
							//Hier noch den Schaden an R�stung und Leben filtern
...
/*						if(AnzahlAkt >= ReservedSpace)
					...
					if(strcmp(action, " triggered ") == 0)
					{
						...
						//Es wird �berpr�ft ob es eine "g�ltige" Aktion ist, dann wird der Name
						//ausgelesen und an Players.NewAction wird der Name, die Aktion und die Anzahl �bergeben
						while(z < AKTIONENANZAHL && !Found)
						{
							...
							if(strcmp(Aktionen[z].text, StrFilterReverse(Line, 1, '\"')) == 0)
							{
								...
								//Ein tempor�rer String wird erstellt und die aktuelle Zeile hineinkopiert
					...
					if(strcmp(action, " joined team ") == 0)
					{
						...
						//Spieler zu den aktuellen hinzuf�gen und Datum + Zeit
...
/*							if(AnzahlAkt >= ReservedSpace)
	...
	//					QMessageBox::information(0, WelchesTeam, Name);
				...
				//�ndert die AktNamen Liste -> L�scht den Namen, der disconnected hat
				//Wichtig f�r die Rundenberechnung
				...
				if(strcmp(StrFilter(Line, 1, '\"'), "Round_End") == 0)
				{
					...
					for(z = 0; z < AnzahlAkt; z++)
					{
						...
						//Guckt welcher Spieler �berlebt hat
					...
					//Wenn nur ein SPieler �berlebt hat, kriegt er Last man Standing
					if(AuslagerungAnz == 1)
						Players->NewAction(Auslagerung[0].c_str(), "LastManStanding", 1);
					//Alle Spieler die �berlebt haben, kriegen ein RoundSurvived gutgeschrieben
...
//				QMessageBox::information(0, StrFilter(temp, 1, '\"'), StrFilterReverse(temp, 1, '\"'));
		...
		//Spielzeit f�r die Leute berechnen, die den Server nicht verlassen haben!
	...
	else
	{
		...
		sprintf(Nachricht, "Datei: %s konnte nicht ge�ffnet werden", LogFile);
		throw MyException(Nachricht);
	}
}
