...
//Speichert die Anzahl der Sortierungen, die man f�r Write Player File braucht, um die Spieler
//Einmal nach Kills, Deaths, KDRate... sortieren zu k�nnen
...
struct AKTIONEN
{
	...
	bool ShowTotal;
	int TeamSkill;
	//Zeigt an ob es Allgemein oder zu TS oder zu CT geh�rt
	int Status;
};
...
{
private :
	//Platform spezifisch : anderes Image Verzeichnis f�r Linux benutzen
	char ImagePath[201];
	...
	/*Was man f�r die Schadensanzeige braucht :*/
	...
	vector <int> Skill;
	vector <float> KDRate;
	//Steht die Sortierung der Spieler drin, f�r die Kills, Deaths, KDRate... siehe SortCount oben
	...
	vector <int> NumAward;
	//Insgesamte Ausf�hrung einer Action
	vector <int> CountofAction;
	
	//Speichert den Namen der einen Award hat(f�r die Floataktionen)
	vector <string> FloatPlayerAward;
	//Speichert die Anzahl mit der der Spieler den Award hat(f�r die Floataktionen)
	...
	int LogFiles;
	//Wenn es 0 ist werden nicht gerankte Spieler nicht gel�scht und in der Statistik ohne Link geschrieben
	//Wenn es 1 ist werden nicht gerankte Spieler gel�scht und sind somit nicht in der Statistik vorhanden
	//In beiden F�llen wird keine eigene Spielerdatei von einem nicht gerankten Spieler erstellt.
	...
	//Findet die H�chste Anzahl eines vectors raus und schreibt den Namen in Player
	...
	//Speichert die NamenTags, die NICHT benutzt werden d�rfen
	...
	//Wird ben�tigt damit man Spieler zusammenf�gen kann, das ganze passiert unter WelcherSpieler
...
public :
	...
	//Liefert den Index der Waffe zur�ck
	inline int WelcheWaffe(const char *Waffe);
	//Liefert den Index der Aktion zur�ck
	...
	//Liefert die meisten Kills dieser Waffe zur�ck und schreibt den Namen in Player
